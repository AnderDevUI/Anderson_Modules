...
if AnderDeveloperESP then
	AnderDeveloperESP:Exit()
end

--// Settings

getgenv().AnderDeveloperESP = {
	DeveloperSettings = {
		Path = "Ander Developer/Ander ESP/Configuration.cfg",
...
local Environment = getgenv().AnderDeveloperESP
...
local CoreFunctions = {
	...
	GenerateHash = function(Bits)
		...
		for _ = 1, Bits do
			Result ..= ("Ander_ESP")[mathrandom(1, 2) == 1 and "upper" or "lower"](stringchar(mathrandom(97, 122)))
		end
...
local UtilityFunctions = {
	...
	WrapObject = function(self, Object, PseudoName, Allowed, RenderDistance)
		assert(self, "Ander_ESP > UtilityFunctions.WrapObject - Internal error, unassigned parameter \"self\".")
		...
		if not Entry.IsAPlayer then
			if not pcall(function()
				...
				warn("Ander_ESP > UtilityFunctions.WrapObject - Attempted to wrap object of an unsupported class type: \""..(__index(Entry.Object, "ClassName") or "N / A").."\"")
...
Environment.UnwrapAll = function(self) -- METHOD | (<void>) => <void>
	assert(self, "Ander_ESP.UnwrapAll: Missing parameter #1 \"self\" <table>.")
...
Environment.Restart = function(self) -- METHOD | (<void>) => <void>
	assert(self, "Ander_ESP.Restart: Missing parameter #1 \"self\" <table>.")
...
Environment.Exit = function(self) -- METHOD | (<void>) => <void>
	assert(self, "Ander_ESP.Exit: Missing parameter #1 \"self\" <table>.")
	...
	if self:UnwrapAll() then
		...
		for Index, _ in next, Environment do
			getgenv().AnderDeveloperESP[Index] = nil
		end

		LoadESP = nil; Recursive = nil; Loaded = false

		getgenv().AnderDeveloperESP = nil
	end
end
...
Environment.UpdateConfiguration = function(DeveloperSettings, Settings, Properties) -- (<table> DeveloperSettings, <table> Settings, <table> Properties) => <table> New Environment
	assert(DeveloperSettings, "Ander_ESP.UpdateConfiguration: Missing parameter #1 \"DeveloperSettings\" <table>.")
	assert(Settings, "Ander_ESP.UpdateConfiguration: Missing parameter #2 \"Settings\" <table>.")
	assert(Properties, "Ander_ESP.UpdateConfiguration: Missing parameter #3 \"Properties\" <table>.")

	getgenv().AnderDeveloperESP.DeveloperSettings = DeveloperSettings
	getgenv().AnderDeveloperESP.Settings = Settings
	getgenv().AnderDeveloperESP.Properties = Properties

	Environment = getgenv().AnderDeveloperESP

	return Environment
end

Environment.LoadConfiguration = function(self) -- METHOD | (<void>) => <void>
	assert(self, "Ander_ESP.LoadConfiguration: Missing parameter #1 \"self\" <table>.")
...
Environment.SaveConfiguration = function(self) -- METHOD | (<void>) => <void>
	assert(self, "Ander_ESP.SaveConfiguration: Missing parameter #1 \"self\" <table>.")
